...
use ash::util::*;
use ash::vk;
use sandgears::{find_memorytype_index, offset_of, record_submit_commandbuffer, ExampleBase};
...
fn main() {
    unsafe {
        ...
        let vertices = [
            Vertex {
                pos: [20.0, 20.0, 200.0, 400.0],
                uv: [0.0, 0.0],
            },
            Vertex {
                pos: [20.0, 400.0, 200.0, 400.0],
                uv: [0.0, 1.0],
            },
            Vertex {
                pos: [400.0, 400.0, 200.0, 400.0],
                uv: [1.0, 1.0],
            },
            Vertex {
                pos: [400.0, 20.0, 200.0, 400.0],
                uv: [1.0, 0.0],
            },
        ];
        ...
        let windowSize = base.window.inner_size();
        let ortho_matrix = nalgebra_glm::ortho_no(
            0.0,
            windowSize.width as f64,
        ...
        let uniform_ortho_buffer = vk::BufferCreateInfo {
            size: mem::size_of_val(&ortho_matrix) as u64,
            usage: vk::BufferUsageFlags::UNIFORM_BUFFER,
            sharing_mode: vk::SharingMode::EXCLUSIVE,
            ..Default::default()
        };
        let uniform_ortho_buffer = base
            .device
            .create_buffer(&uniform_ortho_buffer, None)
            .unwrap();
        let uniform_ortho_buffer_memory_req = base
            .device
            .get_buffer_memory_requirements(uniform_ortho_buffer);
        let uniform_ortho_buffer_memory_index = find_memorytype_index(
            &uniform_ortho_buffer_memory_req,
            &base.device_memory_properties,
            vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT,
        )
            .expect("Unable to find suitable memory type for the vertex buffer.");

        let uniform_ortho_buffer_allocate_info = vk::MemoryAllocateInfo {
            allocation_size: uniform_ortho_buffer_memory_req.size,
            memory_type_index: uniform_ortho_buffer_memory_index,
            ..Default::default()
        };
        let uniform_ortho_buffer_memory = base
            .device
            .allocate_memory(&uniform_ortho_buffer_allocate_info, None)
            .unwrap();
        let orth_uniform_ptr = base
            .device
            .map_memory(
                uniform_ortho_buffer_memory,
                0,
                uniform_ortho_buffer_memory_req.size,
                vk::MemoryMapFlags::empty(),
            )
            .unwrap();
        let mut uniform_ortho_aligned_slice = Align::new(
            orth_uniform_ptr,
            mem::align_of::<Vector3>() as u64,
            uniform_ortho_buffer_memory_req.size,
        );
        uniform_ortho_aligned_slice.copy_from_slice(&[ortho_matrix]);
        base.device.unmap_memory(uniform_ortho_buffer_memory);
        base.device
            .bind_buffer_memory(uniform_ortho_buffer, uniform_ortho_buffer_memory, 0)
            .unwrap();

        let image = image::load_from_memory(include_bytes!("../resources/App/a1.png"))
            .unwrap()
            .to_rgba8();
        ...
        let uniform_ortho_buffer_descriptor = vk::DescriptorBufferInfo {
            buffer: uniform_ortho_buffer,
            offset: 0,
            range: mem::size_of_val(&ortho_matrix) as u64,
        };
        ...
        let write_desc_sets = [
            ...
            vk::WriteDescriptorSet {
                ...
                descriptor_count: 1,
                descriptor_type: vk::DescriptorType::UNIFORM_BUFFER,
                p_buffer_info: &uniform_ortho_buffer_descriptor,
                ..Default::default()
            },
        ];
        ...
        base.device.free_memory(uniform_ortho_buffer_memory, None);
        base.device.destroy_buffer(uniform_ortho_buffer, None);
